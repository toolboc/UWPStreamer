...
namespace XAMStreamer.Services
{
    public class NTR : INotifyPropertyChanged
    {
        ...
        public async void NTRRemoteplayConnect()
        {
            ...
            await socket.StartListeningAsync(serverPort);
        }

        private async void NTRRemoteplayReadJPEG(object sender, Sockets.Plugin.Abstractions.UdpSocketMessageReceivedEventArgs e)
        {
        ...
        private async Task TryDisplayImage(List<byte> screenBuffer, int screen)
        {
            Device.BeginInvokeOnMainThread(
                () =>
                {
                    ...
                    Stream stream = new MemoryStream(screenBuffer.ToArray());
                    stream.Seek(0, SeekOrigin.Begin);

                    if (screen == 1)
                        Screen1 = ImageSource.FromStream(() => stream);
                    else
                        Screen0 = ImageSource.FromStream(() => stream);
